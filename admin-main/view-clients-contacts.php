...
if($_REQUEST['act'] == "del")
	{
		...
		$row = mysql_query($query) or die(mysql_error());
			...
			document.location.href="view-clients-contacts.php?msg=delete";
			</script>
			<?php
	}
...
$targetpage = "view-clients-contacts.php";	

$limit = 10;

$query = "SELECT COUNT(*) as num from registration where registration_status = 0";
...
// Get page data
$sql = "select * from registration where registration_status = 0 order by registration_date ASC LIMIT $start, $limit";
...
while($rowLogo = mysql_fetch_array($result))
	{
		...
		if($rowLogo['registration_status'] == 1)
			{
				$statusLogo = '<a href="view-clients-contacts.php?lid='.$rowLogo['registration_id'].'&status=1"><img src="images/icon-active.jpg" alt="Deactive" width="20" height="20" /> </a>';
			}
		else
			{
				$statusLogo = '<a href="view-clients-contacts.php?lid='.$rowLogo['registration_id'].'&status=0"><img src="images/icon-deactive.jpg" alt="Deactive" width="20" height="20" /> </a>';
			}
		...
		$dataLogo .= '
			...
			<td><a href="full-profile.php?rid='.$rowLogo['registration_id'].'" target="_blank">'.ucfirst($rowLogo['registration_fname']).'</a></td>
					...
					<td>'.$rowLogo['registration_contact_address'].'</td>
					<td>'.$rowLogo['registration_mobile1'].'</td>
					<td>'.$rowLogo['registration_mobile2'].'</td>
					<td>'.$rowLogo['registration_std'].' '.$rowLogo['registration_telephone'].'</td>
					<td>'.$dateANDtime.'</td>
					<td align="center">
						'.$statusLogo.' 
						...
						<a href="view-clients-contacts.php?act=del&lid='.$rowLogo['registration_id'].'" class="headerLink" onclick="return ask_ques();"><img src="images/icon-delete.jpg" alt="Delete" width="20" height="20" /></a>
					</td>
				</tr>
		';
	}
...
$result_quali = mysql_query($selectQualification) or die(mysql_error());
	...
	include("html/admin/view-clients-contacts.html");

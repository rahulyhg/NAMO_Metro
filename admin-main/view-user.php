...
if($_REQUEST['act'] == "del")
	{
		...
		$query="delete from customer where cust_id =".$_REQUEST['lid'];
		$row = mysql_query($query) or die(mysql_error());
			?>
			<script language="javascript" type="text/javascript">
			document.location.href="view-user.php?msg=delete";
			</script>
			<?
	}

$query = "SELECT COUNT(*) as num from customer";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];

/* Setup vars for query. */
	$targetpage = "view-user.php"; 	//your file name  (the name of this file)
...
$sql = "select * from customer order by cust_date  DESC LIMIT $start, $limit";
...
while($rowLogo = mysql_fetch_array($result))
	{	
		$dateANDtime = date('F j, Y', $rowLogo['cust_date']);
		$noLogo++;
		$dataLogo .= '
				<tr>
					<td align="center">'.$noLogo.'</td>
					<td><a href="view-customer.php?uid='.$rowLogo['cust_id'].'">'.$rowLogo['cust_fullname'].'</a></td>
					<td>'.$rowLogo['cust_email'].'</td>
					<td>'.$dateANDtime.'</td>
					<td align="center">						
						<a href="view-user.php?act=del&lid='.$rowLogo['cust_id'].'" class="headerLink" onclick="return ask_ques();"><img src="images/icon-delete.jpg" alt="Delete" width="20" height="20" /></a>
...
if($_REQUEST['msg']=='padd')
	...
	include("html/admin/view-user.html");

...
class PHPExcel_Reader_CSV extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	 ...
	 * @var	string
	 */
	private $_inputEncoding	= 'tis620_thai_ci';
	 ...
	 */
	public function setInputEncoding($pValue = 'tis620_thai_ci')
	{
		...
		while (($rowData = fgetcsv($fileHandle, 0, $this->_delimiter, $this->_enclosure)) !== FALSE) {
			...
			foreach($rowData as $rowDatum) {
				if ($rowDatum != '' && $this->_readFilter->readCell($columnLetter, $currentRow)) {
					...
					if ($this->_inputEncoding !== 'tis620_thai_ci') {
						$rowDatum = PHPExcel_Shared_String::ConvertEncoding($rowDatum, 'tis620_thai_ci', $this->_inputEncoding);
					}

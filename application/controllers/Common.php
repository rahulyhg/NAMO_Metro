	...
	public function register_user()
	{
		...
		if($img_data['userfile']['error'] == 0){
				...
				if (!$this->upload->do_upload()){
					...
					$source_img = $config['upload_path'].$filename;
					$destination_img = $config['upload_path'].$filename;
         	...
         	if(!empty($ans))
         	{
         		...
         		if(!empty($res))
         		{
         			...
         			$this->send_registrion_mail($reg_id[0]['registration_id']);
                                ...
                                ob_clean();
         			//header('http://www.saiprasadvivah.net/common/payment_success/'.$payu_id);
					redirect('https://www.saiprasadvivah.net/common/payment_success/'.$payu_id, 'refresh');
                                //header('/common/payment_success/'.$payu_id);   
		...
		if((isset($_POST["status"]) && $_POST["status"] =="failure") || ($_POST["status"] =="pending")) 
		{
			...
			$update_data = array('registration_payment_status'=>$status,
         	...
         	if(!empty($ans))
         	{
         		...
         		$data['pay_email'] = $email;
         		$data['pay_fname'] = $_POST["firstname"];
         		$data['pay_name_on_card'] = isset($_POST["name_on_card"]) ? $_POST["name_on_card"] : "NULL";
         		$data['pay_net_amount_debit'] = $_POST["net_amount_debit"];
         		$data['pay_bank_ref_num'] = $_POST["bank_ref_num"];
         		$data['pay_date_time'] = $datetime;
         		//print_r($data);exit;
...
redirect('https://www.saiprasadvivah.net/common/payment/'.$id.'/'.$msg, 'refresh');
         			//header('/common/payment/'.$id.'/'.$msg);
                                  //header('https://www.saiprasadvivah.net/common/payment/'.$id.'/'.$msg);
         		}
         	}
		}
		else
	...
	public function send_registrion_mail($registration_id)
	{
		...
		$password = $record[0]['registration_password'];
	        ...
	        $email_body .='<br/>';
	        $email_body .='<ul>';
            $email_body .='<li><b style="color:red;">Internet Facility:</b><br/>We will publish your information along with photographs on this website.Registration only At Kurla Office as Following Office Time: Sat 4:00pm to 8:00pm & Sun 9:00am to 12:00pm. Pay Rs.2500/- Till Marriage Fixed (One time only)</li>';
            ...
            $email_body.='</ul>';
            $email_body.='<p>
                           <b style="color:red;">If you are already paid RS.2500/- then please ignored this message.</b>
                           </p>';
            $email_body.='<footer>';
	...
	public function login_action($search_gender=""){
		...
		public function edit_profile($active="")
		{
			$registration_id = $this->session->userdata('registration_id');
			...
			if(!empty($registration_id)){
				...
				//echo "<pre>";print_r($record['active']);exit();
		...
		public function profile_advance_search()
		{
			...
			//$total_page=ceil(count($result['data']) / $perpage);
			//echo '<pre>';print_r($total_page);exit;
			$this->load->view('forms/search_listing',$result);
		}
		///////////////////////////////////
			...
			if(!empty($registration_id)){
				...
				$data = array(
					...
					'registration_subgroup' => $form_data["registration_subgroup"],
                         'registration_pan_adhar' => $form_data["registration_pan_adhar"]
				 );
				if($img_data['user_profile']['error'] == 0)
				{
		...
		public function astro_details()
		{
			...
			if(!empty($registration_id)){
				...
				$data = array(
					...
					'registration_birth_date' => date("Y-m-d",strtotime($form_data["registration_birth_date"])),
        ...
        {
        	$id=1;//for about_us page
            $data['membership']=$this->mmaster->show_about_us($id);
        	$this->load->view('forms/membership',$data);
        }
         public function melawa_view_more($id)            
        {
            $data['melawa_details']=$this->mmaster->show_view_more_melawa($id);
            $data['melawa_img']=$this->mmaster->show_melawa_img($id);
            //print_r($data);exit;

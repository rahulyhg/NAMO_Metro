...
function update_rgister_profile_pic_status(id,value)
    ...
    ////////////////////
    function genarate_otp_for_login(reg_id,redirect_path="",login_type="")
    {
        var path = base_url+"common/genarate_otp_for_login/"+reg_id;
        $.ajax({
            type:'POST',
            url:path,
            //data:"email="+email+"&pass="+pass,
            ...
            success:function(resp)
            {
                ...
                if(resp != 1)
                {
                    ...
                    data += '<form id="opt_form" >';
                    data += '<div class="col-sm-12 col-sm-offset-0 alert_msg1" style="color:white;"></div>';
                    data += '<input name="registration_id" type="hidden" class="form-control signup_input" id="registration_id" value="'+reg_id+'"/>';
                    // data += '<input name="registration_otp" type="text" class="form-control signup_input" id="registration_otp" value="'+resp+'"/>';
                    data += '<input name="user_otp" type="text" class="form-control signup_input" id="user_otp"/>';
                    data += '<button type="button" class="pull-left btn btn-verify" style="background-color:#ff6600; color:white; margin-top:9%;" onclick="genarate_otp_for_login('+reg_id+')">Resend OTP</button>';
                    data += '<button type="button" class="pull-right btn btn-verify" style="background-color:red; color:white; margin-top:9%;" onclick="verify_otp_for_login(\''+redirect_path+'\',\''+login_type+'\')">Submit</button>';
                    data += '</form>';
                    $(".modal-title").html('Enter OTP Which You got On Your Register Mobile');
                    $(".modal-body").html(data);
                    $("#myModal").modal('show');
                }
                else if(resp == 0)
                {
                     var msg = 'Something goes Wrong.. Please try again.';
                     display_alert('err',msg); 
                } 
             }
        });
...
function destroy_msg_count()
{
    var path = base_url+"common/destroy_msg_count";
    $.ajax({
        type:'POST',
        url:path,
        success:function(resp)

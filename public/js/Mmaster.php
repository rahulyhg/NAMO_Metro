<?php
	...
	class Mmaster extends CI_model{
		...
		{
		   $show_iamge=$this->db->query("SELECT registration_id, registration_lname, registration_highest_qualification,registration_age,registration_city,registration_photo,registration_gender FROM registration WHERE registration_status = 1 ORDER BY registration_id DESC limit 40")->result_array();
		   return $show_iamge;
		}
		//////////////////
		public function search_all_matches()
		{
			...
			if(isset($_GET['registration_gender']) && $_GET['registration_gender'] != "" && $_GET['registration_gender'] != "null"){
				...
				$extra .= " WHERE registration_gender = '$registration_gender' ";
				...
				$registration_religion = $_GET['registration_religion'];
				$extra .= " WHERE registration_religion = '$registration_religion'";
				...
				$extra .= " WHERE registration_age BETWEEN $from AND $to";
				...
				$registration_mother_tongue = $_GET['registration_mother_tongue'];
				$extra .= " WHERE registration_mother_tongue = '$registration_mother_tongue'";
				...
				$registration_country = $_GET['registration_country'];
				$extra .= "WHERE registration_country = '$registration_country'";
				}
			
			//print_r("SELECT * from registration $extra");exit();
			 //print_r($extra);exit;
			$record = $this->db->query("SELECT * from registration $extra")->result_array();
			return $record;
		...
		public function advance_search_matches()
		{
			...
			if(isset($_GET['TxtGender']) && $_GET['TxtGender'] != "" && $_GET['TxtGender'] != "null"){
				...
				$extra .= " WHERE registration_gender = '$TxtGender' ";
				...
				if(isset($_GET['TxtEducationLevel']) && $_GET['TxtEducationLevel'] != "" && $_GET['TxtEducationLevel'] != "null")
				{
				...
				if(isset($_GET['city']) && $_GET['city'] != "" && $_GET['city'] != "null")
				{
				$city = $_GET['city'];
				...
				{
				$extra .= "WHERE registration_height BETWEEN $height_from AND $height_to";
			...
			else if(isset($_GET['TxtEducationLevel']) && $_GET['TxtEducationLevel'] != "" && $_GET['TxtEducationLevel'] != "null")
				{
				$TxtEducationLevel = $_GET['TxtEducationLevel'];
				$extra .= "WHERE registration_education_level = '$TxtEducationLevel'";
				...
				{
				$TxtMotherTongue = $_GET['TxtMotherTongue'];
				$extra .= "WHERE registration_mother_tongue = '$TxtMotherTongue'";
			...
			else if(isset($_GET['city']) && $_GET['city'] != "" && $_GET['city'] != "null")
				{
				$city = $_GET['city'];
				$extra .= "WHERE registration_city = '$city'";
				
			}

			else if(isset($_GET['fname']) && $_GET['fname'] != "" && $_GET['fname'] != "null")
				{
				$fname = $_GET['fname'];
				$extra .= "WHERE registration_fname = '$fname'";
			...
			else if(isset($_GET['lname']) && $_GET['lname'] != "" && $_GET['lname'] != "null")
				{
				$lname = $_GET['lname'];
				$extra .= "WHERE registration_lname = '$lname'";
				...
				$pid = $_GET['pid'];
				$pid = substr($pid,3);
				$extra .= "WHERE registration_id = '$pid'";
